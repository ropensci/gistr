...
gist_create_git <- function(files = NULL, description = "", public = TRUE, browse = TRUE,
  ...
  # ra <- tryCatch(git2r::remote_add(git, "gistr", sprintf("https://gist.github.com/%s.git", gst$id)), error = function(e) e)
  if (is(ra, "error")) message(strsplit(ra$message, ":")[[1]][[2]])
  # push up files
  git2r::push(git, "gistr", "refs/heads/master", force = TRUE)
  # refresh gist metadata
  gst <- gist_refresh(gst)
...
gist_refresh <- function(x) {
  x <- as.gist(x)
  while (x$files[[1]] == ".gistr" && length(x$files) == 1) {
    x <- gist(x$id)
  }
  x
}
