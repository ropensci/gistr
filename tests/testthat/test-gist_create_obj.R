...
test_that("gist_create_obj works from a data.frame", {
  ...
  expect_is(df_1, "gist")
  expect_named(df_1$files, "file.md")
  expect_true(df_1$public)
  
  expect_is(df_3, "gist")
  expect_named(df_3$files, "file.md")
  expect_true(df_3$public)
  expect_true(identical(mtcars$mpg, jsonlite::fromJSON(df_3$files$file.md$content)$mpg))
...
test_that("gist_create_obj works from a matrix", {
  ...
  expect_named(mat_1$files, "file.md")
...
test_that("gist_create_obj works from a list", {
  ...
  expect_named(list_1$files, "file.md")
